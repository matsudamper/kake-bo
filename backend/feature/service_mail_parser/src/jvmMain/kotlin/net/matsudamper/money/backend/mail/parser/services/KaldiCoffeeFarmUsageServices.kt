...
internal object KaldiCoffeeFarmUsageServices : MoneyUsageServices {
    ...
    override fun parse(
        ...
        val orderNumber = orderNumberLine?.substringAfter("[注文番号]")?.trim() ?: ""

        val orderDateLine = lines.firstOrNull { it.contains("[ご注文日]") }
        val orderDate = orderDateLine?.substringAfter("[ご注文日]")?.trim() ?: ""

        val paymentMethodLine = lines.firstOrNull { it.contains("[お支払方法]") }
        val paymentMethod = paymentMethodLine?.substringAfter("[お支払方法]")?.trim() ?: ""

        val totalAmountLine = lines.firstOrNull { it.contains("・お支払金額") }
        val totalAmountString = totalAmountLine?.substringAfter("・お支払金額")?.trim() ?: ""
    ...
    private fun extractProducts(plain: String): List<Product> {
        ...
        val startIndex = lines.indexOfFirst { it.contains("[ご注文商品]") }
        ...
        for (line in productSection) {
            when {
                line.startsWith("・商品名（商品コード）") -> {
                    currentProduct?.let { products.add(it) }
