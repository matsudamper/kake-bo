...
val javaToolchainVersion = libs.versions.javaToolchain.get().toInt()
val graalVmLanguageVersion = JavaLanguageVersion.of(javaToolchainVersion)
...
val nativeBuildArgs = listOf(
    ...
    "--initialize-at-run-time=io.lettuce",
...
tasks.matching { it.name == "nativeCompile" || it.name == "nativeTestCompile" }.configureEach {
    doFirst {
        ...
        fixSymlink(binPath.resolve("native-image.cmd"), svmBinPath.resolve("native-image.cmd"))
    }
}
